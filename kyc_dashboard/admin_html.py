...
function LoginView({ onLogin }) { const [username, setUsername] = useState("admin"); const [password, setPassword] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  const handleLogin = useCallback(async () => { setError(""); setLoading(true); try { const res = await fetch(API + "/api/auth/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, password }), }); if (!res.ok) throw new Error("Invalid credentials"); const json = await res.json(); localStorage.setItem("auth_token", json.token); localStorage.setItem("auth_role", json.role); onLogin(json.token, json.role);} catch (err) { setError(err.message || "Login failed"); } setLoading(false); }, [username, password, onLogin]);
...
function App() { const [isAuthed, setIsAuthed] = useState(false); const [token, setToken] = useState(null); const [role, setRole] = useState(null); const [view, setView] = useState("worklist"); const [cases, setCases] = useState([]); const [loading, setLoading] = useState(false);
  useEffect(() => { const storedToken = localStorage.getItem("auth_token"); const storedRole = localStorage.getItem("auth_role"); if (storedToken && storedRole) { setToken(storedToken); setRole(storedRole); setIsAuthed(true); loadCases(storedToken);} }, []);
  ...
  const handleLogin = useCallback((newToken, newRole) => { setToken(newToken); setRole(newRole); setIsAuthed(true); setView("worklist"); loadCases(newToken); }, [loadCases]);
  const handleLogout = useCallback(async () => { try { await fetch(API + "/api/auth/logout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token }), }); } catch {} localStorage.removeItem("auth_token"); localStorage.removeItem("auth_role"); setIsAuthed(false); setToken(null); setRole(null); setCases([]); }, [token]);
