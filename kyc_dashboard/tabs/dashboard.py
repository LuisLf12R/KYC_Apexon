...
# ── Banker-only full-page iframe (Flask sidecar, no Streamlit chrome) ─────────

def _render_banker_iframe(user: Dict[str, Any], logger: Any) -> None:
    institutions = _get_available_institutions()
    initial_data = {
        "institutions": [{"id": iid or "", "label": lbl} for iid, lbl in institutions],
        "sidecarUrl": "http://127.0.0.1:8502",
    }
    html = _build_banker_html(initial_data)
    st_components.html(html, height=980, scrolling=False)


def _build_banker_html(initial_data: Dict[str, Any]) -> str:
...
const SIDECAR = INIT.sidecarUrl || "http://127.0.0.1:8502";
...
function BatchUploadView({ onBack }) {
  ...
  const dsOptions = [
    ...
    {v:"id_verifications",l:"ID Verifications"},{v:"transactions",l:"Transactions"},
    ...
    <div>
      ...
      >
        ...
        <label className="btn primary" style={{cursor:"pointer"}}>
          Browse files
          <input type="file" multiple accept=".pdf,.png,.jpg,.jpeg,.csv,.xlsx,.xls,.json,.jsonl" onChange={e=>addFiles(e.target.files)} style={{display:"none"}}/>
        </label>
      </div>
...
function App() {
  ...
  const runQueue = useCallback(async institutionId => {
    ...
    try {
      ...
      setError(err.message || "Could not reach KYC sidecar. Is it running?");
    }
    setLoading(false);
  }, []);
  ...
  return (
    <div style={{minHeight:"100%",background:"var(--bg-sunken)",display:"flex",flexDirection:"column"}}>
      <Toolbar
        institutions={INIT.institutions || []}
        onRun={runQueue}
        onUpload={goUpload}
        loading={loading}
      ...
      <div style={{flex:1,padding:"20px 24px",overflowY:"auto"}}>
        ...
        {view === "worklist" && data && !loading && (
          <WorklistView data={data} onOpenCase={openCase}/>
        )}

        {view === "case" && (
          <CasePanel c={activeCase} onBack={goBack}/>
        )}
      </div>
    </div>
  );
...
def render(user: Dict[str, Any], role: str, logger: Any) -> None:
    if role == "Banker":
        _render_banker_iframe(user, logger)
        return
